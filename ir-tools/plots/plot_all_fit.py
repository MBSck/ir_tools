...
if __name__ == "__main__":
    path = Path("/Users/scheuck/Data/model_results/2024-07-30/results_model_10:34:24")
    ...
    plot_component_mosaic(components, dim, 0.1, wavelength.value, norm=0.2,
                          savefig=fit_plot_dir / "mosaic_model.pdf", zoom=25)
    plot_fit(components=components, savefig=fit_plot_dir / "fit_results.pdf")
    plot_intermediate_products(dim, wavelength, components, component_labels, save_dir=fit_plot_dir)
    ...
    # labels, units = np.load(path / "labels.npy"), np.load(path / "units.npy")
    # plot_corner(sampler, labels, units, savefig=path / "corner.pdf")
    # plot_chains(sampler, labels, units, savefig=path / "chains.pdf")
