...
def compute_temperature_grid(
    ...
    combined_opacities = (1 - weights[:, np.newaxis]) * silicate_opacities + weights[
        :, np.newaxis
    ] * continuum_opacities
    radiation_fields = (
    ...
    p_in = np.trapz(
        (
            combined_opacities[:, np.newaxis, :] * radiation_fields[np.newaxis, ...]
        ).value,
        nu.value,
        axis=-1,
    )
    ...
    temps = np.arange(
        temperature_range[0], temperature_range[1] + 1, temperature_steps
    )[:, np.newaxis]
    bb = (
        4
        * np.pi
        * u.sr
        * BlackBody(temperature=temps * u.K)(wavelengths)[np.newaxis, ...]
    )
    p_out = integrate_values(
        (bb * combined_opacities[:, np.newaxis, :]).value, nu.value, njobs=ncores
    )
    difference = p_in[..., np.newaxis] - p_out[:, np.newaxis, :]
    return weights, radii, temps.flatten()[np.argmin(np.abs(difference), axis=-1)]


# TODO: Do the temperature calculation proper
