...
def combine_chopped_non_chopped(
    directory: Path, dates: Optional[List[str]] = None, **kwargs
) -> None:
    ...
    fits_files = [
        fits_file for fits_file in fits_files if "CHOPPED" not in fits_file.name
    ]
    ...
    for fits_file in tqdm(fits_files):
        ...
        with fits.open(new_file, "update") as hdul:
            ...
            if "FLXCHOP" in header:
                continue
            header["FLXCHOP"] = (
                f"Chopped flux from the file {fits_file.name}" "added as the total flux"
            )
...
def calc_vis_from_corrflux(
    input_corrflux_file, input_totalflux_file, outfile_path, propagate_fluxerr=True
):
...
def calculate_vis(
    directory: Optional[Path] = None, propagate_fluxerr: Optional[bool] = True, **kwargs
) -> None:
    ...
    for fits_file in list(directory.glob("*.fits")):
        ...
        calc_vis_from_corrflux(
            fits_file, fits_file, new_file, propagate_fluxerr=propagate_fluxerr
        )
        plot = Plotter(new_file, save_dir=plot_dir)
