...
def query_catalog(object_name: str, catalog: str,
    ...
    if catalog == "wise":
        ...
        if flux:
            lband = True if index == 1 else False
            w_err_ratio = w_err / w
            w = wise_to_flux(w, lband)
            w_err = w * w_err_ratio
...
def plot_multi_epoch(object_name: str, flux: bool = False,
                     flux_ranges: List[List[float]] = [[None, None], [None, None]]) -> None:
    ...
    for index, (wise_index, ax) in enumerate(zip([1, 3], axarr.flatten())):
        wise_data = query_catalog(object_name, "wise", wise_index, flux)
        ...
        ax.set_ylabel("Luminosity (mag)" if not flux else "Flux (Jy)")
        ax.set_xlabel("Time (UTC)")
        if flux:
            ax.set_ylim(flux_ranges[index])
        ax.set_title(f"{object_name.upper()} WISE{wise_index}")
