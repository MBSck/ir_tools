...
if __name__ == "__main__":
    ...
    wl_op, silicate_opacity = np.load(
        opacity_dir / "hd142527_boekel_qval_silicates.npy"
    )
    ...
    grid_hdu = fits.BinTableHDU(
        Table(data={"weights": [temperatures.weights], "radii": [temperatures.radii]}),
        name="grid".upper(),
    )
    opacity_hdu = fits.BinTableHDU(
        Table(
            data={
                "silicates": [wl_op, silicate_opacity],
    ...
    const_kappa_temp = np.sqrt((3.46 * u.Rsun).to(u.au) / (2 * temperatures.radii)) * (
        6500 * u.K
    )
    ...
    for index, weight in enumerate(np.arange(0, 1.1, 0.1)):
        ax.plot(
            temperatures.radii,
            interp1d(temperatures.weights, temperatures.values, axis=0)(weight),
            label=f"{weight:.1f}",
        )
        bx.plot(
            wl_cont,
            (1 - weight) * silicate_opacity + weight * cont_opacity,
            label=f"{weight:.1f}",
        )
