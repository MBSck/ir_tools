...
def query_catalog(object_name: str, catalog: str, index: int, flux: bool = False):
...
def plot_multi_epoch(
    object_name: str,
    flux: bool = False,
    flux_ranges: List[List[float]] = [[None, None], [None, None]],
) -> None:
    """Plot the multi-epoch WISE data for the specified object."""
    # akari_data = query_catalog(object_name, "akari")

    # NOTE: The WISE bands to be plotted
    wise_bands = [1, 2, 3, 4]
    errorbar_kwargs = {"fmt": "o", "ecolor": "gray", "capsize": 5}
    wise_data = [
        query_catalog(object_name, "wise", index, flux) for index in wise_bands
    ]

    for index, data in enumerate(wise_data):
        plt.errorbar(
            data["time"],
            data["value"],
            data["error"],
        ...
        if flux:
            plt.ylim(flux_ranges[index])

    plt.tick_params(axis="x", labelsize=6, rotation=45)
    plt.ylabel("Luminosity (mag)" if not flux else "Flux (Jy)")
    plt.xlabel("Time (UTC)")
    plt.legend()
